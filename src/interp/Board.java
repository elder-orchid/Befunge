...
public class Board {
	...
	public void step(){
		...
		if(!finished) {
			...
			// If it's a hex input
			else if (((int)curVal - (int)'0' < 10 && (int)curVal - (int)'0'>=0) || ((int)curVal - (int)'A' < 6 && (int)curVal - (int)'A'>=0) || ((int)curVal - (int)'a' < 6 && (int)curVal - (int)'a'>=0)) {
				...
				// Special char
				switch(curVal) {
				// In the order of esolang.org/wiki/Befunge
